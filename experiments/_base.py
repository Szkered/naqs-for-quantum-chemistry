...
def _run(
  ...
  '''
    Load molecule
    '''
  molecule, qubit_hamiltonian = load_molecule(
    molecule_fname, hamiltonian_fname=hamiltonian_fname, verbose=True
  )
  ...
  for i in range(num_experiments):
    ...
    n_alpha, n_beta = molecule.get_n_alpha_electrons(
    ), molecule.get_n_beta_electrons()
    m_s = np.abs(n_alpha - n_beta) // 2

    if (m_s != 0) and restrict_to_ms:
      print(
        "S!=0 and we are restricting ourselves to ms=S --> turning off use_amp_spin_sym as this is not helpful."
      )
    ...
    if m_s == 0 or restrict_to_ms:
      ...
      print(
        f"Initialised Hilbert space with N={hilbert.N}, and {hilbert.size} physically valid configurations."
      )
    else:
      n = (n_alpha + n_beta)
      ...
      print(
        f"Configured valid numbers of alpha/beta electrons : {n_alpha}/{n_beta}."
      )
      hilbert_args = {
        "N": N,
        "N_alpha": n_alpha,
    ...
    wavefunction_args = {
      'qubit_ordering':
        qubit_ordering,  # 1: default, 0: random, -1:reverse, list:custom
    ...
    wavefunction = NAQSComplex_NADE_orbitals(hilbert, **wavefunction_args)
    ...
    if loadH:
      if n_excitations_max is None:
        ham_fname = os.path.join(
          molecule_fname,
          f"{os.path.split(molecule_fname)[-1]}_sparse_hamiltonian.npz"
        )
      else:
        ham_fname = os.path.join(
          molecule_fname,
          f"{os.path.split(molecule_fname)[-1]}_{n_excitations_max}exc_sparse_hamiltonian.npz"
        )
    else:
      ham_fname = None
    ...
    if (presolveH and (hilbert.size < 50000)):
      ...
      print(
        f"Numerically diagonalised ground state energy : {eig_val[0].real:.6f}."
      )
    ...
    if continue_experiment:
      ...
      if presolveH:
        with torch.no_grad():
          print(
            "Pre-trained amplitudes:",
            opt.wavefunction.amplitude(
              hilbert.get_subspace(**opt.subspace_args)
            )[:5]
          )
          print(
            "Pre-trained phases:",
            opt.wavefunction.phase(hilbert.get_subspace(**opt.subspace_args)
                                  )[:5]
          )
          print(
            f"Pre-trained energy : {opt.calculate_energy(normalise_psi=True):.4f} Hartree (HF : {molecule.hf_energy:.4f} Hartree)"
          )
    ...
    summary.append(
      f"{len(opt.sampled_idxs)}/{opt.hilbert.size} ({100*len(opt.sampled_idxs)/opt.hilbert.size:.2f}%) of basis elements sampled at least once."
    )
    ...
    for lab, E in zip(["VMC", "VMC+FCI"], [E_loc_min, eig_val]):
      ...
      summary.append(
        f'\tBelow Hartree-Fock ({molecule.hf_energy:.5f} Hartree) : {E < molecule.hf_energy}'
      )
      summary.append(
        f'\tBelow CCSD ({molecule.ccsd_energy:.5f} Hartree) : {E < molecule.ccsd_energy}'
      )

      if molecule.fci_energy is not None:

        summary.append(
          f'\tBelow FCI ({molecule.fci_energy:.5f} Hartree) : {E < molecule.fci_energy}'
        )
        ...
        if molecule.fci_energy + chem_acc > E:
          ...
          summary.append(
            f"\tNot reaching chemical accuracy...\n\t\tNAQS energy : {E:.5f} >= {(molecule.fci_energy + chem_acc):.5f}"
          )
...
def get_parser(
  ...
  parser = argparse.ArgumentParser(
    description='Run experimental script.', allow_abbrev=True
  )

  parser.add_argument(
    '-m', '--molecule', nargs='?', default=molecule, help='The molecule folder'
  )
  ...
  parser.add_argument(
    ...
    '--hamiltonian_fname',
    nargs='?',
    default=hamiltonian_fname,
    help='The qubit hamiltonian pkl file location.'
  )

  parser.add_argument(
    '-o', '--out', nargs='?', default=out, help='The output folder'
  )
  ...
  parser.add_argument(
    '-weight_by_psi',
    default=reweight_samples_by_psi,
    action='store_true',
  ...
  parser.add_argument(
    '-full_mask_psi',
    default=full_mask_psi,
    action='store_true',
    help='Mask the wavefunction to the only restricted Hilbert space.'
  )

  parser.add_argument(
    '-lr', nargs='?', default=lr, type=float, help='The learning rate.'
  )
  ...
  parser.add_argument(
    '-n_lut', nargs='?', default=n_lut, type=int, help='The number of luts.'
  )
  ...
  parser.add_argument(
    '-s', '--seed', nargs='?', default=seed, type=int, help='Training seed.'
  )
...
def run_from_parser(parser):
  ...
  if args.no_mask_psi and args.full_mask_psi:
    raise Exception(
      "Invalid option combination: at most one of -no_mask_psi and -full_mask_psi can be specified."
    )
  ...
  print(
    f"Running experimental script: {__file__}\nResults will be saved to: {exp_name}/"
  )
  print("\nscript options:")
  for label, val in zip(
    [
      "molecule_fname", "hamiltonian_fname", "exp_name", "num_experiments",
      "load (pre_trained model)", "continue_experiment", "qubit_ordering", "lr",
      "lr_lut", "n_samps", "n_samps_max", "n_unq_samps_max", "n_unq_samps_min",
      "weight_by_psi", "no_mask_psi", "full_mask_psi", "n_train", "n_pretrain",
      "n_lut", "n_hid", "n_layer", "n_hid_phase", "n_layer_phase",
      "output_freq", "save_freq", "comb_amp_phase", "no_amp_sym", "phase_sym",
      "single_phase", "loadH", "overwriteH", "presolveH", "n_excitations_max",
      "no_restrictedH", "reset_optimizer", "verbose", "seed"
    ], [
      molecule_fname, args.hamiltonian_fname, exp_name, args.number, args.load,
      args.cont, args.qubit_ordering, args.lr, args.lr_lut, args.n_samps,
      int(args.n_samps_max),
      int(args.n_unq_samps_max),
      int(args.n_unq_samps_min
         ), args.weight_by_psi, args.no_mask_psi, args.full_mask_psi,
      args.n_train, args.n_pretrain, args.n_lut, args.n_hid, args.n_layer,
      args.n_hid_phase, args.n_layer_phase, args.output_freq, args.save_freq,
      args.comb_amp_phase, args.no_amp_sym, args.phase_sym, args.single_phase,
      args.loadH, args.overwriteH, args.presolveH, args.n_excitations_max,
      args.no_restrictedH, args.resetOpt, args.verbose, args.seed
    ]
  ):
    print(f"\t{label} : {val}")
  print("")
