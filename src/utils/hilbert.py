...
class _HilbertFull(_HilbertBase):
  ...
  def __init__(
    ...
    if make_basis:
      ...
      if self.verbose:
        print("\tMaking basis", end="...")
      self.basis_states, self.basis_idxs = self.get_basis(
        ret_states=True, ret_idxs=True
      )
    else:
      if self.verbose:
        print("\tMaking __basis_idxs", end="...")
      self.basis_idxs = self.to_idx_tensor(
        self.__pad_idxs(self.__make_basis_idxs())
      )
    if self.verbose:
      print("done.")
  ...
  def __check_config(self, N_up, N_alpha, N_beta, N_occ, N_exc_max):
    if ((N_up is not None) and (N_alpha is not None) and (N_beta is not None)):
  ...
  def get_subspace(
    ...
    N_up, N_alpha, N_beta, N_occ, N_exc_max = self.__check_config(
      N_up, N_alpha, N_beta, N_occ, N_exc_max
    )
    ...
    if key in self.subspaces:
      ...
      mask = None
      if ((N_up is not None) and (N_alpha is None) and (N_beta is None)):

        mask = (space_bool.sum(1) == N_up)

      elif (
        (N_up is not None) and (N_alpha is not None) and (N_beta is not None)
      ):
  ...
  def get_basis(
    self, ret_states=True, ret_idxs=False, use_restricted_idxs=False
  ):
  ...
  def idx2state(self, idx, use_restricted_idxs=False):
    if self.N_occ > 0:
      ...
      if not valid_idx.all():
        raise Exception(
          "The following idxs are not valid in the configured Hilbert space:\n",
          idx[~valid_idx]
        )
...
class _HilbertRestricted(_HilbertBase):
  ...
  def __init__(
    ...
    self.size = int(
      comb(math.ceil(self.N / 2), N_alpha) *
      comb(math.floor(self.N / 2), N_beta)
    )
    ...
    if self.N <= 30:
      ...
      full2restricted_basis_idxs[self.restricted2full_basis_idxs] = np.arange(
        len(self.restricted2full_basis_idxs)
      )
      self.full2restricted_basis_idxs = self.to_idx_tensor(
        full2restricted_basis_idxs
      )
    else:
      self.use_full2restricted_lut_arr = False
      self.full2restricted_basis_idxs = defaultdict(
        lambda: -1,
        np.stack(
          [
            self.restricted2full_basis_idxs,
            np.arange(len(self.restricted2full_basis_idxs))
          ]
        ).T
      )
    ...
    self.subspaces = {
      (None, None): (self.basis_states.clone(), self.basis_idxs.clone())
    }

  def __prepare_basis(self):

    alpha_set_bits = np.array(
      list(combinations(np.arange(0, self.N, step=2), self.N_alpha))
    )
    beta_set_bits = np.array(
      list(combinations(np.arange(1, self.N, step=2), self.N_beta))
    )

    # alphabeta_set_bits = np.array(list(product(alpha_set_bits, beta_set_bits))).reshape(-1, self.N_up)
    alphabeta_set_bits = np.array(
      [np.concatenate(x) for x in product(alpha_set_bits, beta_set_bits)]
    )

    restricted_basis = np.zeros(
      (len(alphabeta_set_bits), self.N), dtype=self._state_np_dtype
    )

    restricted_basis[np.broadcast_to(
      np.arange(len(alphabeta_set_bits))[:, None], alphabeta_set_bits.shape
    ), alphabeta_set_bits] = 1
    ...
    return (
      self.to_state_tensor(restricted_basis),
  ...
  def get_subspace(
    ...
    if key in self.subspaces:
      ...
      mask = None
      space_states, space_idxs = self.basis_states.clone(
      ), self.basis_idxs.clone()
      if N_occ is not None:
  ...
  def state2idx(self, state, use_restricted_idxs=False):
    if type(state) is np.ndarray:
      state = torch.from_numpy(state)
    state_clamped = state.clamp_min(0)
    idxs = self.to_idx_tensor(
      (state_clamped * self._idx_basis_vec).sum(dim=-1, keepdim=True)
    )
  ...
  def __full2restricted_idx_dic_lut(self, idx):
    ...
    if len(idx) > 1:
      idx = np.fromiter(
        itemgetter(*idx.squeeze())(self.full2restricted_basis_idxs),
        self._idx_np_dtype,
        count=len(idx)
      )
    else:
...
class _HilbertPartiallyRestricted(_HilbertBase):
  ...
  def __prepare_basis(self):
    ...
    for n_alpha, n_beta in zip(self.N_alpha, self.N_beta):
      alpha_set_bits = np.array(
        list(combinations(np.arange(0, self.N, step=2), n_alpha))
      )
      beta_set_bits = np.array(
        list(combinations(np.arange(1, self.N, step=2), n_beta))
      )

      # alphabeta_set_bits = np.array(list(product(alpha_set_bits, beta_set_bits))).reshape(-1, self.N_up)
      _alphabeta_set_bits = np.array(
        [np.concatenate(x) for x in product(alpha_set_bits, beta_set_bits)]
      )

      _restricted_basis = np.zeros(
        (len(_alphabeta_set_bits), self.N), dtype=self._state_np_dtype
      )

      _restricted_basis[np.broadcast_to(
        np.arange(len(_alphabeta_set_bits))[:, None], _alphabeta_set_bits.shape
      ), _alphabeta_set_bits] = 1
  ...
  def __check_config(self, N_up, N_alpha, N_beta, N_occ, N_exc_max):
    N_up, N_alpha, N_beta, N_occ, N_exc_max = (
      self.__to_arr(N_up), self.__to_arr(N_alpha), self.__to_arr(N_beta),
      self.__to_arr(N_occ), self.__to_arr(N_exc_max)
    )
    ...
    assert all(
      N_up == N_alpha + N_beta
    ), f"N_up ({N_up}) must be the sum of N_alpha ({N_alpha}) and N_beta ({N_beta})"
...
class Unitaries():
  default_unitaries = {
    "X": np.matrix([[1, 1], [1, -1]]) / np.sqrt(2),
    "Y": np.matrix([[1j, 1], [1, 1j]]) / np.sqrt(2),
    "Z": np.matrix([[1, 0], [0, 1]]),
    "-X": -1 * np.matrix([[1, 1], [1, -1]]) / np.sqrt(2),
    "-Y": -1 * np.matrix([[1j, 1], [1, 1j]]) / np.sqrt(2),
    "-Z": -1 * np.matrix([[1, 0], [0, 1]])
  }
  ...
  def __init__(self, unitaries=None, qubit_encoding=Encoding.SIGNED):
    ...
    assert type(
      user_unitaries
    ) == dict, "Must pass unitaries as a dictionary object."
  ...
  def get_rotated_coeffs(self, q_state, q_basis, encoding=None):
    ...
    coeffs = [
      self.rotated_qubits[q_idx][q_basis]
      for q_idx in self.__qubit2idx(q_state.squeeze(-1), encoding)
    ]
  ...
  def rotate_state(self, states, state_bases, encoding=None):
    ...
    for qubit_idx, (s, b) in enumerate(
      zip(states.transpose(1, 0, 2), state_bases)
    ):
      if b != "Z":
