...
def plot_training(vmc_log, molecule, window=None, print_summary=True):
  ...
  iters_local_energy_var, local_energy_var = np.array(
    vmc_log[LogKey.E_LOC_VAR]
  ).T
  ...
  def _plot_energy_convergence(ax):
    mask = np.ones((50,)) / 50
    ax.plot(
      np.convolve(iters_local_energy, mask, mode='valid'),
      np.convolve(local_energy, mask, mode='valid'),
      label=r"$\langle E_\mathrm{loc} \rangle$",
      color=cols["local energy"],
      linewidth=0.75,
      ls='--'
    )
    # ax.plot(iters_local_energy, local_energy, label=r"$\langle E_\mathrm{loc} \rangle$", color=cols["local energy"], linewidth=0.75, ls='--')
    ax.plot(
      iters_energy, energy, label=r"$E$", color=cols["energy"], linewidth=1
    )
  ...
  def _plot_energy_error(ax):
    ...
    ax.axhline(
      molecule.fci_energy + chem_acc - e_min,
      0,
      1,
      ls='--',
      color=cols["chem acc"],
  ...
  if print_summary:
    ...
    print(
      f'\tBelow Hartree-Fock ({molecule.hf_energy:.5f} Hartree) : {E_min < molecule.hf_energy}'
    )
    print(
      f'\tBelow CCSD ({molecule.ccsd_energy:.5f} Hartree) : {E_min < molecule.ccsd_energy}'
    )
    print(
      f'\tBelow FCI ({molecule.fci_energy:.5f} Hartree) : {E_min < molecule.fci_energy}'
    )
...
def plot_wavefunction(
  ...
  if hilbert_args is None:
    ...
    states, states_idx = wavefunction.hilbert.get_subspace(
      *hilbert_args, ret_idxs=True
    )
