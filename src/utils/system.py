...
def load_molecule(fname, hamiltonian_fname=None, verbose=True):
  ...
  if verbose:
    ...
    print(
      f"\t{n_qubits} qubits (orbitals), with {molecule.n_electrons} electrons ({n_alpha}/{n_beta} alpha/beta)."
    )
  ...
  for obj in gc.get_objects():
    try:
      if torch.is_tensor(obj) or (hasattr(obj, 'data')):
        if obj.is_cuda:
          print(
            f"type : {type(obj)}, size : {obj.size()}, dtype : {obj.dtype}, device : {obj.device}, has_grads : {obj.grad is not None}"
          )
    except:
      pass
  for i in range(4):
