...
class PauliHamiltonian():
  ...
  def get(
    ...
    if hamiltonian_fname is not None:
      hamiltonian_fname, hamiltonian_fname_npz = PauliHamiltonian._format_fnames(
        hamiltonian_fname
      )
      if os.path.exists(hamiltonian_fname):
        pauli_hamiltonian = _PauliHamiltonianDynamic(
          hilbert, qubit_hamiltonian, restricted_idxs, n_excitations_max,
          verbose, dtype
        )
        pauli_hamiltonian.load(hamiltonian_fname)
      elif os.path.exists(hamiltonian_fname_npz):
        pauli_hamiltonian = _PauliHamiltonianFrozen(
          hilbert, qubit_hamiltonian, hamiltonian_fname_npz, restricted_idxs,
          n_excitations_max, verbose, dtype
        )

    if pauli_hamiltonian is None:
      pauli_hamiltonian = _PauliHamiltonianDynamic(
        hilbert, qubit_hamiltonian, restricted_idxs, n_excitations_max, verbose,
        dtype
      )
...
class __PauliHamiltonianBase(ABC):
  ...
  def get_coupled_state_idxs(self, state_idxs, return_unique=False):
    '''Return the coupled indexes (columns) corresponding to each state (row),
        ...
        '''
    coupled_idxs = [
      self.H.indices[self.H.indptr[idx]:self.H.indptr[idx + 1]]
  ...
  def load_H(self, fname):
    ...
    if self.H.dtype is not np.dtype(self.dtype):
      print(
        f"\tLoaded dtype ({self.H.dtype}) does not match expected dtype ({self.dtype})."
        + f"\tCasting loaded matix to {self.dtype}"
      )
    print(
      f"\tMatrix has size {self.H.shape} and {self.H.nnz} non-zero elements."
    )
...
class _PauliHamiltonianDynamic(__PauliHamiltonianBase):
  ...
  def __init__(
    ...
    print(
      "\tpre-processing Hamiltonian terms and coupling lookups", end="...\n\t"
    )
    self.XY_sites_idx, self.YZ_sites_idx, self.couplings = self.__calc_coupling_info(
    )

    self._unique_XY_sites_idx, self._unique2all_XY_sites_idx = np.unique(
      self.XY_sites_idx, return_inverse=True
    )
    self._unique_YZ_sites_idx, self._unique2all_YZ_sites_idx = np.unique(
      self.YZ_sites_idx, return_inverse=True
    )

    self._unique_XY_sites_idx = self.hilbert.to_idx_array(
      self._unique_XY_sites_idx
    )
    self._unique2all_XY_sites_idx = self.hilbert.to_idx_array(
      self._unique2all_XY_sites_idx
    )
    ...
    print(
      f"Pauli Hamiltonian has K={len(self.couplings)} terms:", end="\n\t\t--> "
    )
    print(
      f"there are {len(self._unique_XY_sites_idx)} unique XY bit-strings (1 if v_k,n = {{X,Y}}, 0 if v_k,n = {{I,Z}})),",
      end="\n\t\t--> "
    )
    print(
      f"there are {len(self._unique_YZ_sites_idx)} unique YZ bit-strings (1 if v_k,n = {{Y,Z}}, 0 if v_k,n = {{I,X}})).\n"
    )
  ...
  def update_H(self, state_idx, check_unseen=True, assume_unique=False):
    ...
    if not self._frozen_H:
      ...
      if self.verbose:
        t = time.time()
        print(
          f"Sampled {len(state_i_idx)} unique states (dtype={state_i_idx.dtype}):",
          end="\n\t--> "
        )
      if check_unseen:
        state_i_idx = np.setdiff1d(
          state_i_idx, self._cached_idxs, assume_unique=assume_unique
        )
        if self.verbose:
          print(
            f"gives {len(state_i_idx)} un-cached states ({time.time()-t:.4f}s).",
            end="\n\t--> "
          )
      ...
      P_k_bits_by_unique_YZ_sites = np.bitwise_and(
        state_i_idx[:, None], self._unique_YZ_sites_idx[None, :]
      )
      if self.verbose:
        print(
          f"P_k_bits_by_unique_YZ_sites has shape {P_k_bits_by_unique_YZ_sites.shape} ({P_k_bits_by_unique_YZ_sites.dtype}) when considering unique YZ bit strings) ({time.time()-t:.4f}s).",
          end="\n\t--> "
        )
      P_k_by_unique_YZ_sites = popcount_parity(P_k_bits_by_unique_YZ_sites)
      if self.verbose:
        print(
          f"P_k_by_unique_YZ_sites has shape {P_k_by_unique_YZ_sites.shape} when considering unique YZ bit strings) ({time.time()-t:.4f}s).",
          end="\n\t--> "
        )
      ...
      j_idx_full = np.bitwise_xor(
        state_i_idx[:, None], self._unique_XY_sites_idx[None, :]
      ).ravel()
      # j_idx_full = np.bitwise_xor(state_i_idx[None, :], self._unique_XY_sites_idx[:, None]).ravel()
      if self.verbose:
        print(
          f"Calculated coupled states (i.e. j_idx_full : {j_idx_full.shape}, {j_idx_full.dtype}) ({time.time()-t:.4f}s).",
          end="\n\t--> "
        )
      ...
      if self.verbose:
        print(
          f"Converted states from full --> restricted idxs ({time.time()-t:.4f}s).",
          end="\n\t--> "
        )
      ...
      if self.verbose:
        print(
          f"Calculated physical_coupling_mask (M={M}, Kxy={Kxy}, len(H_ij) - len(physical_coupling_mask)={M*Kxy - len(physical_coupling_mask)})({time.time()-t:.4f}s).",
          end="\n\t--> "
        )

      # Finally, all that remains is to calculate the couplings.
      H_ij = get_Hij_cy(
        state_i_idx, self._unique_XY_sites_idx, self._unique2all_XY_sites_idx,
      ...
      if self.verbose:
        print(
          f"Calculated couplings (i.e. H_ij's) ({H_ij.shape}) ({time.time()-t:.4f}s).",
          end="\n\t--> "
        )

      H_ij = H_ij[physical_coupling_mask]
      i_idx, j_idx = i_idx[physical_coupling_mask // Kxy
                          ], j_idx[physical_coupling_mask]

      if self.verbose:
        print(
          f"Apply mask to Hamiltonian data ({time.time()-t:.4f}s).",
          end="\n\t--> "
        )

      H_new = csr_matrix(
        (H_ij, (i_idx, j_idx)), shape=(self.hilbert.size, self.hilbert.size)
      )

      if self.verbose:
        print(
          f"Created new sparse Hamiltonian with {H_new.nnz} non-zero elements ({time.time()-t:.4f}s).",
          end="\n\t--> "
        )
  ...
  def __calc_coupling_info(self):
    ...
    for coupling_idx, (term, coupling) in enumerate(
      self.qubit_hamiltonian.terms.items()
    ):
    ...
    XY_sites = self.hilbert.to_idx_array(
      (self.hilbert._idx_basis_vec.unsqueeze(0) * torch.stack(XY_sites)).sum(1)
    )
    YZ_sites = self.hilbert.to_idx_array(
      (self.hilbert._idx_basis_vec.unsqueeze(0) * torch.stack(YZ_sites)).sum(1)
    )
...
class _PauliHamiltonianFrozen(__PauliHamiltonianBase):
  ...
  def unfreeze_H(self, *args, **kwargs):
    '''Allow further updates to H.
        ...
        '''
    raise NotImplementedError(
      "Can't unfreeze the PauliHamiltonian.  Presumably this Hamiltonian was loaded from a file"
      +
  ...
  def update_H(self, *args, **kwargs):
    ...
    if not self._frozen_H:
      raise NotImplementedError(
        "Can't update the PauliHamiltonian.  Presumably this Hamiltonian was loaded from a file"
        +
        "and so was created without any of the dynamic back-end for calculating new couplings."
      )
