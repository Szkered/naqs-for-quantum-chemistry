...
from src.optimizer.utils import KFACOptimizer, LogKey
...
class OptimizerBase(ABC):
  ...
  def __init__(
    ...
    restricted_idxs = self.hilbert.get_subspace(
      ret_states=False, ret_idxs=True, **self.subspace_args
    )

    self.pauli_hamiltonian = PauliHamiltonian.get(
      self.hilbert,
      qubit_hamiltonian,
    ...
    if pre_compute_H and not self.pauli_hamiltonian.is_frozen():
      ...
      self.pauli_hamiltonian.update_H(
        restricted_idxs, check_unseen=False, assume_unique=True
      )
  ...
  def reset_optimizer(self, cond_idx=None):
    ...
    if self.optimizer_callable in opt_list:
      self.optimizer = self.optimizer_callable(
        self.wavefunction.model, **self.optimizer_args
      )
      ...
      if type(self.optimizer_args) is not dict:
        ...
        self.optimizer = self.optimizer_callable(
          self.wavefunction.conditional_parameters(cond_idx),
          **self.optimizer_args
        )
    ...
    if self.scheduler_callable is not None:
      ...
      self.scheduler = self.scheduler_callable(
        self.optimizer, **self.scheduler_args
      )
      print("done.")
    else:
      self.scheduler = None
    ...
    self.__grad_norms = [
      deque([], self.grad_clip_memory_length)
  ...
  def calculate_energy(self, normalise_psi=None):
    ...
    self.pauli_hamiltonian.update_H(
      states_idx, check_unseen=True, assume_unique=True
    )
    ...
    energy = psi.conj().dot(
      sparse_dense_mv(self.pauli_hamiltonian.get_restricted_H(), psi)
    )
  ...
  def calculate_local_energy(
    ...
    states_idx,
    psi=None,
    set_unsampled_states_to_zero=True,
    ret_complex=False
  ):
    ...
    if psi is None:
      psi = self.wavefunction.psi(
        self.hilbert.idx2state(states_idx, use_restricted_idxs=False),
        ret_complex=True
      )
    else:
      psi = psi.detach()
    ...
    if set_unsampled_states_to_zero:
      local_energy = (
        sparse_dense_mv(self.pauli_hamiltonian.get_H(states_idx), psi) / psi
      ).conj()
  ...
  def _SGD_step(
    ...
    # 2.Compute the local energies for each state (no network gradients requried): E_loc(|s>).
    e_loc = self.calculate_local_energy(
      states_idx.squeeze(), psi=cplx.exp(log_psi.detach())
    )
    if self.verbose:
    ...
    if sample_weights is None:
      if self.reweight_samples_by_psi:
        ...
        raise NotImplementedError(
          "Re-weighting by the number of samples is not yet implemented."
        )
    ...
    exp_op = 2 * cplx.real(
      sample_weights * cplx.scalar_mult(log_psi, e_loc_corr)
    ).sum(axis=0)
    ...
    with torch.no_grad():
      ...
      if log_psi_eval is not None:
        e_loc = self.calculate_local_energy(
          states_idx.squeeze(), psi=cplx.exp(log_psi_eval.detach())
        )
      ...
      local_energy_variance = (
        (cplx.real(e_loc) - local_energy).pow(2) * sample_weights.squeeze()
      ).sum()
  ...
  def _clip_grads(self):
    if self.grad_clip_factor is not None:
      for grad_norms, group in zip(
        self.__grad_norms, self.optimizer.param_groups
      ):
        max_norm = self.grad_clip_factor * np.mean(grad_norms).item(
        ) if len(grad_norms) > 0 else 1e3
        ...
        norm = torch_utils.clip_grad_norm_(
          group['params'], max_norm, norm_type=2
        )
        try:
          norm = norm.item()
        except:
  ...
  def load(self, fname="energy_optimizer", quiet=False):
    ...
    try:
      ...
      print(
        f"\twavefunction not found (expected at {checkpoint['wavefunction:fname']})"
      )
...
class ExactSamplingOptimizer(OptimizerBase):
  ...
  def __init__(self, num_batches_per_epoch=None, **kwargs):
    kwargs['reweight_samples_by_psi'] = True
    super().__init__(**kwargs)
    self.num_batches_per_epoch = num_batches_per_epoch
    if self.num_batches_per_epoch is None:
      self.__batch_size = len(
        self.hilbert.get_subspace(
          ret_states=False, ret_idxs=True, **self.subspace_args
        )
      )
    else:
  ...
  def solve_H(self, states_idx=None):
    ss, ss_idxs = self.wavefunction.hilbert.get_subspace(
      ret_idxs=True, **self.subspace_args
    )
  ...
  def pre_train(
    ...
    if target_amps is None:
      ...
      target[len(target_amps):] = np.sum(target_amps) / (
        len(ss_idxs) - len(target_amps)
      )
    ...
    for i in range(1, n_epochs + 1):
      for states, amps, mask in dataloader:
        ...
        if mask.sum() > 0:
          ...
          loss = F.binary_cross_entropy(psi[mask], amps[mask])
          loss.backward()
          optimizer.step()
      ...
      if ((i) % output_freq == 0) or (i == 1):
        ...
        print(
          f"\t Epoch {i} : loss = {loss.item():.5e}, |psi|^2 = {psi.norm().detach().item():.3f}, epoch time={time_per_epoch:.2f}s"
        )
        t0 = time.time()
    optimizer.zero_grad()
    print("done.")
  ...
  def run(
    ...
    for i in range(n_epochs):
      ...
      if ((self.n_epochs) % output_freq == 0) or (self.n_epochs == 1):
        ...
        print(
          f"Epoch {self.n_epochs} ({steps_in_epochs} SGD steps) : " +
          f"<E>={energy:.5f}, " + f"<E_loc>={local_energy:.5f}, " +
      ...
      if save_freq is not None:
        if self.n_epochs % save_freq == 0:
          ...
          self.save(
            os.path.join(self.save_loc, f"opt_{self.n_steps}steps"), quiet=True
          )
          print("done.")
    if save_final:
...
class PartialSamplingOptimizer(OptimizerBase):
  ...
  def solve_H(self, n_samps=None, ret_n_samps=True):
    ...
    if n_unq > N_DIAG_MAX:
      print(
        f"Limiting number of sampled states from {n_unq} is to most likely {N_DIAG_MAX}."
      )
      sort_args = np.argsort(counts)
      states = states[sort_args[-N_DIAG_MAX:]]

    print(
      f"Diagonalising sampled Hilbert subspace of size: {n_unq}x{n_unq}",
      end="..."
    )

    H = self.pauli_hamiltonian.get_H(
      self.hilbert.state2idx(states, use_restricted_idxs=False).squeeze()
    )
    eig_val, eig_vec = sp.sparse.linalg.eigs(H, k=1, which='SR', maxiter=1e9)
    eig_vec = eig_vec * np.sign(eig_vec[0])
  ...
  def pre_train(
    ...
    if use_equal_unset_amps:
      amp_other = torch.FloatTensor(
        [
          (np.sum(t**2 for t in target) /
           (len(ss_idxs) - len(target_amps)))**0.5
        ] * self.n_samples
      )
    ...
    for i in range(1, n_epochs + 1):
      ...
      if use_equal_unset_amps:
        ...
        states, counts, probs, sample_log_psi = self.wavefunction.sample(
          self.n_samples
        )
  ...
  def pre_flatten(
    ...
    if not use_sampling:
      states, states_idxs = self.hilbert.get_subspace(
        ret_idxs=True, **self.subspace_args
      )
      if max_batch_size < 0:
      ...
      def run_epoch():
        for j, idx_batch in enumerate(
          torch.randperm(len(states)).chunk(n_batches)
        ):
          optimizer.zero_grad()
          ...
          loss = F.mse_loss(
            log_psi.view(-1),
            torch.FloatTensor([log_amp_target] * len(idx_batch)
                             ).to(log_psi).view(-1)
          )
          loss.backward()
          optimizer.step()
      ...
      def run_epoch():
        ...
        loss = F.mse_loss(
          log_psi.view(-1),
          torch.FloatTensor([log_amp_target] * len(counts)
                           ).to(log_psi).view(-1)
        )
        loss.backward()
        optimizer.step()
    ...
    for i in range(1, n_epochs + 1):
      ...
      if ((i) % output_freq == 0) or (i == 1):
        ...
        print(
          f"\t Epoch {i} : loss = {loss.item():.5e}, |psi|^2 = {log_psi[...,0].exp().norm().detach().item():.3f}, epoch time={time_per_epoch:.2f}s"
        )
        t0 = time.time()
    optimizer.zero_grad()
    print("done.")
  ...
  def run(
    ...
    if self.n_steps == 0:
      self.save(
        os.path.join(self.save_loc, f"opt_{self.n_steps}steps"), quiet=False
      )
    ...
    def get_samples(last_action=0):
      ...
      try:
        states, counts, probs, log_psi = self.wavefunction.sample(
          self.n_samples, max_batch_size=self.n_unq_samples_max
        )
      ...
      if (
        (self.n_samples != self.n_unq_samples_min) and
        (self.n_samples != self.n_samples_max)
      ) or not sampling_completed:
        ...
        if n_unq < self.n_unq_samples_min and last_action >= 0:
          ...
          print(
            f"\t...{n_unq} unique samples generated --> increasing batch size to {self.n_samples/1e6:.1f}M at epoch {self.n_epochs}."
          )
        ...
        # If we want more fewer samples, decrease the sample size.
          ...
          print(
            f"\t...{n_unq} unique samples generated --> decreasing batch size to {self.n_samples/1e6:.1f}M at epoch {self.n_epochs}."
          )
    ...
    for i in range(n_epochs):
      ...
      local_energy, local_energy_variance = self._SGD_step(
        states,
        self.hilbert.state2idx(states, use_restricted_idxs=False),
        log_psi,
      ...
      if ((self.n_epochs) % output_freq == 0) or (self.n_epochs == 1):
        ...
        local_energies = [
          x[1]
          for x in self.log[LogKey.E_LOC][-min(output_freq, self.n_epochs):]
        ]
        ...
        print(
          f"Epoch {self.n_epochs} ({steps_in_epochs} SGD steps with {samp_str} samples ({len(weights)} unq.) : "
          + f"<E>={energy_str}, " +
...
class DensitySamplingOptimizer(OptimizerBase):
  ...
  def __init__(self, dP, dP_steps=None, log_exact_energy=True, **kwargs):
    kwargs['reweight_samples_by_psi'] = True
    super().__init__(**kwargs)
    self.log_exact_energy = log_exact_energy
    self.dP = dP
  ...
  def pre_flatten(
    ...
    for i in range(1, n_epochs + 1):
      ...
      loss = F.binary_cross_entropy(
        psi,
        torch.FloatTensor([amp_target] * len(counts)).to(psi)
      )
  ...
  def run(
    ...
    # Quickly print some information about the number of states and number of mini-batches.
    print(
      f"Training NAQS energy.  Samples will be weighted by their (normalised) wavefunction amplitude squared."
    )
    self.wavefunction.model.clear_cache()
    ...
    for i in range(n_epochs):
      ...
      local_energy, local_energy_variance = self._SGD_step(
        states, self.hilbert.state2idx(states), log_psi, sample_weights=weights
      )
      ...
      if ((self.n_epochs) % output_freq == 0) or (self.n_epochs == 1):
        ...
        print(
          f"Epoch {self.n_epochs} ({steps_in_epochs} SGD steps with dP={dP:.3e} ({len(weights)} unq. states) : "
          + f"<E>={energy:.5f}, " + f"<E_loc>={local_energy:.5f}, " +
          f"var(<E_loc>)={local_energy_variance:.5f}, " +
          f"epoch time={time_per_epoch:.2f}s, total time={self.run_time:.1f}s",
          end="\t\t\t\n"
