...
class LogKey(Enum):
  E = "Energy"
  E_LOC = "Local energy"
  E_LOC_VAR = "Local energy variance"
  N_UNIQUE_SAMP = "Number of unique samples"
  TIME = "Time"
...
class KFACOptimizer(optim.Optimizer):
  ...
  def __init__(
    ...
    self.grad_ouputs_outer = defaultdict(
      dict
    )  # |delta^(l)><delta^(l)|, delta^(l) = dL / dz^(l)

    self.D_inputs = defaultdict(
      dict
    )  # D for eigenval. decomp of |z^(l-1)><z^(l-1)| = QDQ^{-1}
    self.Q_inputs = defaultdict(
      dict
    )  # Q for eigenval. decomp of |z^(l-1)><z^(l-1)| = QDQ^{-1}

    self.D_grads = defaultdict(
      dict
    )  # D for eigenval. decomp of |delta^(l)><delta^(l)| = QDQ^{-1}
    self.Q_grads = defaultdict(
      dict
    )  # Q for eigenval. decomp of |delta^(l)><delta^(l)| = QDQ^{-1}
  ...
  def _save_input(self, module, layer_input):
    ...
    for idx in range(module.n):
      ...
      if module.bias is not None:
        inp_block = torch.cat(
          [inp_block,
           torch.ones(1, inp_block.size(-1)).to(inp_block)], 0
        )
      ...
      self.inputs_outer[module][idx] = self._update_running_stat(
        inp_outer, running_outer
      )
  ...
  def _save_grad_output(self, module, grad_input, grad_output):
    ...
    for idx in range(module.n):
      ...
      self.grad_ouputs_outer[module][idx] = self._update_running_stat(
        grad_outer, running_outer
      )
  ...
  def _update_decomps(self, module, n):
    ...
    try:
      D_grad, Q_grad = torch.symeig(
        self.grad_ouputs_outer[module][n], eigenvectors=True
      )
    except Exception as e:
      print(
        self.grad_ouputs_outer[module][n].min(),
  ...
  def get_param_block(self, module, row_idx):
    ...
    params = module.weight[:max_out, :max_in,
                           row_idx, :min(row_idx + 1, module.n)].contiguous()
  ...
  def get_grad_block(self, module, row_idx):
    max_in = max(module.in_channels[:row_idx + 1])
    max_out = module.out_channels[row_idx]
    g = module.weight.grad.data[:max_out, :max_in,
                                row_idx, :min(row_idx +
                                              1, module.n)].contiguous()
    g = g.view(g.size(0), -1)
    if module.bias is not None:
  ...
  def set_grad_block(self, module, row_idx, new_grads, nu=1):
    ...
    module.weight.grad[:max_out, :max_in,
                       [row_idx], :min(row_idx + 1, module.n)] = w_grads
  ...
  def get_natural_grad(self, module, n=None, damping=0.001):
    ...
    if n is not None:
      ...
      grads = [
        self._get_natural_grad(module, idx, self.get_grad_block(module, idx))
        for idx in range(module.n)
      ]
    return grads
