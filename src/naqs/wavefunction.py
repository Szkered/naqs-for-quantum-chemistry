...
class _NAQSComplex_Base(ABC):
  ...
  def __init__(
    ...
    **kwargs
    ...
    if qubit_ordering == 1:
      ...
      self.qubit2model_permutation = self.model2qubit_permutation = np.arange(
        self.hilbert.N
      )
    else:
      ...
      if qubit_ordering == -1:
        # reverse qubits
        self.qubit2model_permutation = np.concatenate(
          [
            np.arange(self._N_fixed),
        ...
        assert len(
          qubit_ordering
        ) == self.hilbert.N, f"Custom qubit ordering must have length {self.hilbert.N}"
        assert len(
          set(qubit_ordering)
        ) == self.hilbert.N, f"Custom qubit ordering have each qubit exactly once."
        if not (
          qubit_ordering.argsort()[:self._N_fixed] == np.arange(self._N_fixed)
        ).all():
          qubit_ordering = np.concatenate(
            [
              np.arange(self._N_fixed),
              np.delete(
                qubit_ordering,
          ...
          print(
            f"Custom qubit ordering modifed so that the {self._N_fixed} fixed electrons are first."
          )
    ...
    if not self.model.sampling:
      if self._N_fixed > 0:
        ...
        try:
          ...
          self.__fixed_log_psi = torch.zeros(len(s), 2, self._N_fixed,
                                             2).to(log_psi)
  ...
  def _evaluate_log_psi(self, s, gather_state=True):
    ...
    if gather_state:
      model_output = model_output.gather(
        -2,
        self._qubit2idx(s).view(model_output.shape[0], -1, 1,
                                1).repeat(1, 1, 1, 2)
      )
  ...
  def psi(self, s, ret_complex=False, combine_conditionals=True):
    log_psi = self.log_psi(
      s, ret_complex=False, combine_conditionals=combine_conditionals
    )

    psi = torch.stack(
      [
        log_psi[..., 0].exp() * log_psi[..., 1].cos(),
  ...
  def save(self, fname, quiet):
    ...
    if not quiet:
      print("Saved NAQSComplex wavefunction model to {}.".format(fname))
    return fname
  ...
  def load(self, fname):
    ...
    self.qubit2model_permutation = checkpoint[
      'wavefunction:qubit2model_permutation']
    self.model2qubit_permutation = checkpoint[
      'wavefunction:model2qubit_permutation']
    print("Loaded NAQSComplex wavefunction from {}.".format(fname))

  @torch.no_grad()
  def save_psi(self, fname="psi", subspace_args={}, normalise=True):
    basis, basis_idxs = self.hilbert.get_subspace(
      **subspace_args, ret_idxs=True
    )
    ...
    print(
      "Saved psi (as amplitude/phase), basis and basis_idxs to : \n\t{}\n\t{}\n\t{}"
      .format(fname, f"{fname}_basis", f"{fname}_basis_idxs")
    )
...
class NAQSComplex_NADE_orbitals(_NAQSComplex_Base):
  ...
  def __init__(
    ...
    if qubit_ordering == 1:
      ...
      self.qubit2model_permutation = torch.stack(
        [
          torch.arange(self._N_model - 2, -1, -2),
          torch.arange(self._N_model - 1, -1, -2)
        ], 1
      ).reshape(-1)
      self.qubit2model_permutation = torch.stack(
        [
      ...
      self.model2qubit_permutation = np.argsort(self.qubit2model_permutation)
      print(
        "self.qubit2model_permutation (overwritten) :",
        self.qubit2model_permutation
      )
      print(
        "self.model2qubit_permutation (overwritten) :",
        self.model2qubit_permutation
      )
      self.state2model_permutation_shell = torch.arange(
        self._N_model // 2 - 1, -1, -1
      )
    else:
      self.state2model_permutation_shell = self.qubit2model_permutation[1::2
                                                                       ] // 2
    self.model2state_permutation_shell = np.argsort(
      self.state2model_permutation_shell
    )
    print(
      "self.state2model_permutation_shell :", self.state2model_permutation_shell
    )
    print(
      "self.model2state_permutation_shell :", self.model2state_permutation_shell
    )
  ...
  def _evaluate_log_psi(self, s, gather_state=True):
    ...
    if gather_state:
      model_output = model_output.gather(
        -2,
        self.state2shell(s).view(model_output.shape[0], -1, 1,
                                 1).repeat(1, 1, 1, 2)
      )
      # print(f"\nGathered output\n {model_output}")
  ...
  def parameters(self, group_idx=None):
    if group_idx is None or group_idx == -1:
      ...
      params = [
        p for p in self.model.amp_layers[self.model.num_lut:].parameters()
      ]
      params += [
        p for p in self.model.phase_layers[self.model.num_lut:].parameters()
      ]
  ...
  def state2shell(self, s):
    shp = s.shape
    return (
      s.view(shp[0], shp[-1] // 2, 2).clamp_min(0) * torch.LongTensor([1, 2])
    ).sum(-1)
  ...
  def sample(
    ...
    **kwargs
    ...
    if not ret_norm_reg:
      ...
      states, counts, probs, log_psi, norm_reg = self.model(
        num_samples, ret_norm_reg=True
      )
    states = self.hilbert.to_state_tensor(
      states[:, self.model2qubit_permutation]
    )
