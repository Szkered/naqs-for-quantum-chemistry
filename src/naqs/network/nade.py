...
from src.naqs.network.base import (
    ComplexAutoregressiveMachine_Base,
    ComplexCoord,
    InputEncoding,
    NadeMasking,
)
...
class ConvertToAmpPhase(nn.Module):
  ...
  def __init__(
    ...
    ) if amp_activation is not None else None
    self.phase_activation = phase_activation(
    ) if phase_activation is not None else None
    self.combine_amp_phase = combine_amp_phase

  def forward(self, x):
    # amp, phase = torch.split(x, self.n_out, 1)
    amp, phase = torch.split_with_sizes(
      x, [self.n_out_amp, self.n_out_phase], 1
    )
  ...
  def forward(self, x):
    if len(x) <= self.max_batch_size:
      ...
      return torch.cat(
        [
          self.layers(x_batch)
          for x_batch in torch.split(x, self.max_batch_size)
        ]
      )
...
class OrbitalLUT(nn.Module):
  ...
  def __init__(self, num_in=1, dim_vals_in=2, num_out=4, out_activation=None):
    super().__init__()
    ...
    self._idx_basis_vec = torch.LongTensor(
      [self.dim_vals_in**n for n in range(self.num_in)]
    )
  ...
  def _state2idx(self, s):
    return (
      (s.to(self._idx_basis_vec) > 0).long() * self._idx_basis_vec[:s.shape[-1]]
    ).sum(-1)
...
class ComplexAutoregressiveMachine1D_OrbitalNade(
  ComplexAutoregressiveMachine_Base
):
  ...
  def __init__(
    ...
    if (self.n_tot_up is not None
       ) and ((self.n_alpha_up is None) and (self.n_beta_up is None)):
      ...
      print(
        f"\tComplexAutoregressiveMachine1D_OrbitalNade configured for f{self.n_tot_up} total electrons."
      )
      ...
      self.n_alpha_up, self.n_beta_up = np.array(self.n_alpha_up
                                                ), np.array(self.n_beta_up)
      assert len(self.n_alpha_up) == len(
        self.n_beta_up
      ), "Possible options for number of alpha/beta electrons do not match."
      self.n_tot_up = self.n_alpha_up[0] + self.n_beta_up[0]
      assert all(
        (self.n_alpha_up + self.n_beta_up) == self.n_tot_up
      ), "Possible options for number of alpha/beta electrons do not match."
      ...
      self._min_n_set = np.min(
        np.concatenate(
          [
            self.n_alpha_up, self.n_beta_up, self.n_alpha_down, self.n_beta_down
          ]
        )
      )
      print(
        f"\tComplexAutoregressiveMachine1D_OrbitalNade configured for {self.n_tot_up} total electrons ({self.n_alpha_up}/{self.n_beta_up} spin up/down)."
      )

    if self.use_restricted_hilbert:
      assert (
        self.amplitude_encoding is AmplitudeEncoding.LOG_AMP
      ), "Restricted hilbert spaces requires AmplitudeEncoding.LOG_AMP."
    else:
      self._min_n_set = 0
    ...
    if self.combined_amp_phase_blocks:
      print(
        "\tUsing combined amplitude and phase blocks:\n\t\t--> defaulting to amp network params for these blocks."
      )
      if self.use_amp_spin_sym != self.use_phase_spin_sym:
        print(
          "\t\t--> Warning: must use same spin-sym settings for both amplitude and phase when combining them into a single block."
        )
        print(
          f"\t\t\t--> setting self.use_amp_spin_sym=self.use_phase_spin_sym={self.use_amp_spin_sym}"
        )
    ...
    for n in range(self.N // 2):
      ...
      n_in = max(
        1, n_in
      )  # Make sure we have at least one input (i.e. for the first block).
      ...
      if make_with_phase and self.combined_amp_phase_blocks:
        ...
        out_activation = lambda: ConvertToAmpPhase(
          n_out_amp=self._n_out_amp,
    ...
    self._idx_shell_basis_vec = torch.LongTensor(
      [2**n for n in range(self.N // 2)]
    )
  ...
  def modules(self):
    '''
        ...
        '''
    modules = []
    for module in super(ComplexAutoregressiveMachine1D_OrbitalNade,
                        self).modules():
      if len(list(module.children())) > 0:
        continue
      if type(module) in [
        nn.Linear, nn.BatchNorm1d, nn.BatchNorm2d, nn.BatchNorm3d
      ]:
  ...
  def _state2idx(self, s):
    return ((s > 0).long() *
            self._idx_shell_basis_vec[:s.shape[-1]].to(s)).sum(-1)
      ...
      if i >= max(self._min_n_set, 1):
        if self._alpha_beta_restricted:
          ...
          for (_n_alpha_up, _n_alpha_down, _n_beta_up, _n_beta_down) in zip(
            ...
            n_alpha_down, n_beta_down = x_alpha.shape[
              -1] - n_alpha_up, x_beta.shape[-1] - n_beta_up
            ...
            if len(self.n_alpha_up) == 1:
              ...
              for set_idx, is_idx in zip(
                [
                  set_alpha_down_idxs, set_alpha_up_idxs, set_beta_down_idxs,
                  set_beta_up_idxs
                ], [
                  is_alpha_up_idxs, is_alpha_down_idxs, is_beta_up_idxs,
                  is_beta_down_idxs
                ]
              ):
              ...
              already_invalid_mask = (n_alpha_up > _n_alpha_up) | (
                n_alpha_down > _n_alpha_down
              ) | (n_beta_up > _n_beta_up) | (
                n_beta_down > _n_beta_down
              )
              ...
              for set_idx, is_idx in zip(
                [
                  set_alpha_down_idxs, set_alpha_up_idxs, set_beta_down_idxs,
                  set_beta_up_idxs, already_invalid_idxs
                ], [
                  is_alpha_up_idxs, is_alpha_down_idxs, is_beta_up_idxs,
                  is_beta_down_idxs, all_idxs
                ]
              ):
                _mask[set_idx.repeat_interleave(len(is_idx)),
                      is_idx.repeat(len(set_idx))] = 0
          ...
          for set_idx, is_idx in zip(
            [set_down_mask, set_up_mask, not_both_up_mask, not_both_both_mask],
  ...
  def __get_x_ins(self, x, i):
    ...
    for use_spin_sym in [self.use_amp_spin_sym, self.use_phase_spin_sym]:
      ...
      if i == 0:
        x_order = torch.zeros(
          batch_size
        )  # Order of zeros <--> all configured orbitals are the same.
  ...
  def __apply_phase_shifts(self, phase, x_alpha, x_beta, x_tot_order=None):
    if self.use_phase_spin_sym:
      ...
      N_01 = (
        (x_alpha[phase_shift_mask] <= 0) & (x_beta[phase_shift_mask] > 0)
      ).sum(1)
      if phase.dim() > 1:
        N_01.unsqueeze_(-1)
      phase[phase_shift_mask.to(phase.device)
           ] += (math.pi * (N_01.fmod(2))).to(phase.device)
  ...
  def __apply_activations(self, amp_i, phase_i, i, amp_mask=None, masking=None):
    ...
    if (
      masking is NadeMasking.NONE or
      (masking is NadeMasking.PARTIAL and i == (self.N // 2 - 1))
    ):
      amp_mask = None
  ...
  def _forward_sample(
    self, batch_size, ret_output=True, masking=None, max_batch_size=None
  ):
    ...
    for i in range(self.N // 2):
      ...
      amp_i, phase_i = self.__apply_activations(
        amp_i, phase_i, i, _amp_mask, masking
      )
      ...
      with torch.no_grad():
        ...
        new_sample_counts = torch.LongTensor(
          multinomial_arr(counts, probs_i_np)
        )
        ...
        if i == 0:
          ...
          states = torch.cat(
            [
              states.repeat_interleave(num_new_samples_per_state, 0),
              blockidx2spin[new_sample_next_idxs]
            ], 1
          )
  ...
  def _forward_predict(self, x, masking=None):
    ...
    for i in range(num_spins // 2):
      ...
      if masking is not NadeMasking.NONE:
        amp_i, phase_i = self.__apply_activations(
          amp_i, phase_i, i, self.__get_restricted_hilbert_mask(x1, x2, i),
          masking
        )
      else:
        amp_i, phase_i = self.__apply_activations(
          amp_i, phase_i, i, None, masking
        )
